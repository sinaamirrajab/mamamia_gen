...
def save_nifti(tensor, path):
    array = tensor.squeeze().float().detach().cpu().numpy()
    nifti = nib.Nifti1Image(array, affine=np.eye(4))
    nib.save(nifti, path)

def save_all_outputs(images, recon_dict, label_dict, image_names, patient_id, out_dir):
    patient_dir = Path(out_dir) / patient_id
    patient_dir.mkdir(parents=True, exist_ok=True)

    for i, im_name in enumerate(image_names):
        # Save original image
        save_nifti(images[i], patient_dir / im_name)

        # Save all reconstructions
        for key, tensor in recon_dict.items():
            subdir = patient_dir / "images" 
            subdir.mkdir(parents=True, exist_ok=True)

            save_path = subdir / im_name.replace(".nii.gz", f"_{key}.nii.gz")
            save_nifti(tensor[i], save_path)

        # Save all labels
    for key, tensor in label_dict.items():
        subdir = patient_dir / "labels" 
        subdir.mkdir(parents=True, exist_ok=True)
        label_path = subdir / image_names[0].replace(".nii.gz", f"_{key}.nii.gz")
        save_nifti(tensor, label_path)
#%%
...
if __name__ == "__main__":
    ...
    train_dataset = BREASTSpadeDatasetDF_generate(df=df_train, phase=phase)
    ...
    # === Main eval loop ===
    model.eval()
    out_dir = os.path.join(run_dir, "nifti_generation2", str(vae_epoch), phase)

    with torch.no_grad():
        progress_bar = tqdm(enumerate(train_loader), total=len(train_loader), ncols=150, position=0, leave=True)
        for step, batch in progress_bar:
            ...
            # Prepare inputs
            images = torch.cat(
                [batch["tio_sub"][k].data for k in batch["tio_sub"] if "image" in k],
                dim=0
            ).to(device)
            print(f"images shape: {images.shape}")
            labels = batch["tio_sub"]['label'].data.to(device)
            if torch.all(labels == 0):

                print(f"Skipping batch for patient id {batch['data']['patient_id'][0]}: label is empty.")
    
                continue

            # Augmented label variants
            ...
            di_de_labels = elastic_deformation_torchio(di_labels, num_control_points=7, max_displacement=7.5, locked_borders=2)
            er_de_labels = elastic_deformation_torchio(er_labels, num_control_points=7, max_displacement=7.5, locked_borders=2)
            ...
            di_de_labels_oh = one_hot(di_de_labels, 2).to(device)
            er_de_labels_oh = one_hot(er_de_labels, 2).to(device)

            # Run reconstructions
            recon, _, _       = model(images, labels_oh.repeat(images.shape[0], 1, 1, 1, 1))
            di_recon, _, _    = model(images, di_labels_oh.repeat(images.shape[0], 1, 1, 1, 1))
            er_recon, _, _    = model(images, er_labels_oh.repeat(images.shape[0], 1, 1, 1, 1))
            de_recon, _, _    = model(images, de_labels_oh.repeat(images.shape[0], 1, 1, 1, 1))
            di_de_recon, _, _ = model(images, di_de_labels_oh.repeat(images.shape[0], 1, 1, 1, 1))
            er_de_recon, _, _ = model(images, er_de_labels_oh.repeat(images.shape[0], 1, 1, 1, 1))

            # Get file paths
            image_file = Path(batch['data']['image_path'][0])
            image_dir = image_file.parent
            images_path = sorted(os.listdir(image_dir))
            patient_id = batch['data']['patient_id'][0]
            print(f"Processing patient id {patient_id} with image path {images_path}")
            # Convert labels from one-hot to argmax binary (B, D, H, W)
            labels_bin     = torch.argmax(labels_oh, dim=1)
            di_labels_bin  = torch.argmax(di_labels_oh, dim=1)
            er_labels_bin  = torch.argmax(er_labels_oh, dim=1)
            de_labels_bin  = torch.argmax(de_labels_oh, dim=1)
            di_de_labels_bin = torch.argmax(di_de_labels_oh, dim=1)
            er_de_labels_bin = torch.argmax(er_de_labels_oh, dim=1)

            # Collect into dicts for saving
            print(f"size of the reconstructions: {recon.shape}")
            recon_dict = {
                "recon": recon,
                "dilated_recon": di_recon,
                "eroded_recon": er_recon,
                "deformed_recon": de_recon,
                "dilated_deformed_recon": di_de_recon,
                "eroded_deformed_recon": er_de_recon
            }
            print(f"size of the labels: {labels_bin.shape}")
            label_dict = {
                "original": labels_bin,
                "dilated": di_labels_bin,
                "eroded": er_labels_bin,
                "deformed": de_labels_bin,
                "dilated_deformed": di_de_labels_bin,
                "eroded_deformed": er_de_labels_bin
            }

            # Save everything
            save_all_outputs(images, recon_dict, label_dict, images_path, patient_id, out_dir)
            ...
            if step%10== 0: # log to wandb
                ...
                plt.tight_layout()
                os.makedirs(os.path.join(run_dir, "generation/example2" ), exist_ok=True)
                file_path = os.path.join(run_dir, "generation/example2", f"{im_name}_step_{step}.png")
