...
# limitations under the License.
# =========================================================================
# https://github.com/Project-MONAI/MONAI/blob/dev/monai/apps/generation/maisi/networks/diffusion_model_unet_maisi.py
...
class DiffusionModelUNetMaisi(nn.Module):
    ...
    def __init__(
        ...
        include_tumor_type: bool = False,
        num_tumor_type: int = 4,
        include_pcr: bool = False,
        include_acq_time: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.include_tumor_type = include_tumor_type
        self.include_acq_time = include_acq_time
        self.include_pcr = include_pcr
        self.num_tumor_type = num_tumor_type
        ...
        if self.include_spacing_input:
            ...
            new_time_embed_dim += time_embed_dim
        if self.include_tumor_type:
            self.tumor_layer = self._create_embedding_module(self.num_tumor_type, time_embed_dim)
            new_time_embed_dim += time_embed_dim
        if self.include_pcr:
            self.pcr_layer = self._create_embedding_module(1, time_embed_dim)
            new_time_embed_dim += time_embed_dim
        if self.include_acq_time:
            self.acq_time_layer = self._create_embedding_module(1, time_embed_dim)
            new_time_embed_dim += time_embed_dim
    ...
    def _get_input_embeddings(self, emb, top_index, bottom_index, spacing, tumor_type, pcr, acq_time):
        if self.include_top_region_index_input:
        ...
        if self.include_spacing_input:
            ...
            emb = torch.cat((emb, _emb), dim=1)
        if self.include_tumor_type:
            _emb = self.tumor_layer(tumor_type)
            emb = torch.cat((emb, _emb), dim=1)
        if self.include_pcr:
            _emb = self.pcr_layer(pcr)
            emb = torch.cat((emb, _emb), dim=1)
        if self.include_acq_time:
            _emb = self.acq_time_layer(acq_time)
            emb = torch.cat((emb, _emb), dim=1)
        return emb
    ...
    def forward(
        ...
        tumor_type_tensor: torch.Tensor | None = None,
        pcr_tensor: torch.Tensor | None = None,
        acq_time_tensor: torch.Tensor | None = None,
    ) -> torch.Tensor:
        """
        ...
        emb = self._get_input_embeddings(emb, top_region_index_tensor, bottom_region_index_tensor, spacing_tensor, tumor_type_tensor, pcr_tensor, acq_time_tensor)
        h = self.conv_in(x)
